...
def create_testdataloader(opt):
    test_dataset = TestValDataset(opt)
    test_dataloader = DataLoader(
        test_dataset,
        batch_size=opt.batch_size,
        shuffle=False,
        collate_fn=TestValDataset.collate_fn,
    )
...
def get_datapath(dataroot):
    dir_raw = os.path.join(dataroot, 'test\\raw\\img')
    dir_raw_blur = os.path.join(dataroot, 'test\\raw\\blur')
    dir_ref = os.path.join(dataroot, 'test\\ref\\img')
    dir_ref_blur = os.path.join(dataroot, 'test\\ref\\blur')
    dir_ref_locmean = os.path.join(dataroot, 'test\\ref\\local_mean')
    dir_mask = os.path.join(dataroot, 'test\\ref\\segment')
...
class TestDataset(Dataset):
    ...
    def __init__(self, opt):
        ...
        # 调用函数获取数据路径，其中make_dataset出现在util.util里面
        self.raw_size = len(self.data_paths)  # get the size of dataset raw
        # 初始化了一下总数据量和输入输出通道数

    def __getitem__(self, index):
        """Return the total number of images in the dataset."""
        data_path = self.data_paths[index]  # make sure index is within then range
        data = DataProcess(data_path, self.opt)
...
def get_datapath(dataroot):
    dir_raw = os.path.join(dataroot, 'test\\raw')
    raw_paths = glob.glob(dir_raw + '/*.*')
    assert raw_paths, f"No images found in {dir_raw}."
    raw_paths = sorted(
        p for p in raw_paths if p.split(".")[-1].lower() in IMG_FORMATS)
...
def DataProcess(data_path, opt):
    new_size = opt.load_size
    raw = cv2.imread(data_path)
    s = raw.shape[0:2]
    # raw, yx = resize(raw, new_size, bounder=True)
    # raw, yx = hold_resize(raw, new_size, bounder=True)
    raw = resize(raw, new_size)
    y1, y2, x1, x2 = 0, 0, 0, 0
    if s[0] == s[1]:
        y1, x1 = 0, 0
        y2, x2 = new_size, new_size
    elif s[0]>s[1]:
        y1, y2 = 0, new_size
        r = new_size/s[0]
        x1 = int(round((new_size - r * s[1]) / 2 - 0.1))
        x2 = int(x1 + r * s[1])
    elif s[0]<s[1]:
        x1, x2 = 0, new_size
        r = new_size/s[1]
        y1 = int(round((new_size - r * s[0]) / 2 - 0.1))
        y2 = int(y1 + r * s[0])
    ...
    return raw, raw_d, data_path, [y1, y2, x1, x2]


def Get_d(img):
    img_blur = cv2.blur(img, (50, 50))
    img_float = img.astype(np.float32)
    img_blurfloat = img_blur.astype(np.float32)
    img_d = img_float - img_blurfloat
